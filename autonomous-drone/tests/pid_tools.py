#  Class containing the tools to compute a command based on an error and a set
#  of PID parameters
...
class pidTools(object):
	...
	def updateIntegral(self, error, sampleTime):
		...
		if ((self._commandSaturation == 0 or abs(self._command) < self._commandSaturation) and (abs(error) < self._maxError or self._maxError == - 1)):
	...
	def computeCommand(self, error, sampleTime):
		...
		if (self._commandSaturation != 0) :
			self._command = max(min(self._smoothingFactor * self._G * (error + self._Ki * self._integralTerm + self._Kd * self._derivateTerm), self._commandSaturation),-self._commandSaturation) 
		else :
			self._command = self._smoothingFactor * self._G * (error + self._Ki * self._integralTerm + self._Kd * self._derivateTerm)
